...
router.post('/api/projects/save', async ({request,user})=>{
    try {
        ...
        if(!project?.id || !project?.sourceImage) return jsonError(400, 'Project ID and sourceImage both are required');
...
router.post('/api/projects/update', async ({req,user})=>{
    try {
        const userPuter = user.puter;
        ...
        return new Response(JSON.stringify({updated:true, project}), {
            headers: {
                'Content-Type': 'application/json',
                'Access-Control-Allow-Origin': '*'
            }
        ...
        return jsonError(500, 'Failed to update project', {message: error.message || "Unknown error"}); 
    }
})
